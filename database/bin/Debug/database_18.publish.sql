...
GO
PRINT N'Création de Procédure [dbo].[GetAllEventsPaged]...';


GO
CREATE PROCEDURE GetAllEventsPaged
    @PageNumber INT,
    @PageSize INT
AS
BEGIN
    ...
    SELECT EventID, EventName, Location, Description, CategoryID
    FROM (
        SELECT ROW_NUMBER() OVER (ORDER BY EventID) AS RowNum, *
        FROM Events
